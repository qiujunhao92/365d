...
class WC_Gateway_Oceancreditcardonepage extends WC_Payment_Gateway {
    ...
    function payment_fields() {
        ...
        <fieldset>
            ...
            <script>
                ...
                var oceanPlaceOrder = document.getElementById("place_order");
                
                if(oceanPlaceOrder){    
         	               
                    oceanPlaceOrder.onclick = function(){

     	        	    var oceanCheckOnepage = document.getElementById("payment_method_oceancreditcardonepage").checked;
  ...
  public function process_payment($order_id) 
  {
        ...
        //请求网关支付结果
          ...
          }elseif(isset($_POST['mygatewaycarddata'])){
              $card_data = $_POST['mygatewaycarddata'];
          }

        error_log('carddata:'.$card_data);
        $result = $this->op_payment($order,$card_data);
            ...
            if (strtolower($local_signValue) == strtolower($back_signValue)) {
                ...
                if($payment_status == 1){ //支付成功
                    ...
                    if($payment_details !== false){
                        ...
                        wc_add_notice( $payment_details, 'error', [ 'icon' => 'error' ] );

                        $order->add_order_note('Error: '.esc_html(strval($payment_details)));

                        throw new Exception($payment_details);
                    }else{
                        error_log($error_message);
                        wc_add_notice( $error_message, 'error', [ 'icon' => 'error' ] );

                        $order->add_order_note('Error: '.esc_html(strval($error_message)));
    ...
    public function op_payment($order,$card_data)
    {
        ...
        $billing_email		= empty($billing['email']) ? $order_number.'@'.sanitize_text_field($_SERVER['HTTP_HOST']) : $billing['email'];
        //来源网址
        $pay_website = sanitize_text_field($_SERVER['HTTP_HOST']);
        ...
        *==================*/
        $data = array(
            ...
            'ship_firstName'=>empty($shipping['first_name']) ? $this->utf8_substr($billing['first_name'], 0, 64) : $shipping['first_name'],
            'ship_lastName'=>empty($shipping['last_name']) ? $this->utf8_substr($billing['last_name'], 0, 64) : $shipping['last_name'],
            'ship_phone'=>empty($shipping['phone']) ? $this->utf8_substr($billing['phone'], 0, 32) : str_replace( array( '(', '-', ' ', ')', '.' ), '', $billing['phone']),
            'ship_country'=>empty($shipping['country']) ? $this->utf8_substr($billing['country'], 0, 32) : $this->utf8_substr($shipping['country'], 0, 32),
            'ship_state'=>empty($this->get_creditcard_state( $shipping['country'], $shipping['state'])) ? $this->get_creditcard_state( $billing['country'], $billing['state']) : $this->get_creditcard_state($shipping['country'], $shipping['state']),
            'ship_city'=>is_numeric($shipping['city']) ? $billing['city'] : $shipping['city'],
            'ship_addr'=>empty($shipping['address_1'].$shipping['address_2']) ? $billing['address_1'].$billing['address_2'] : $shipping['address_1'].$shipping['address_2'],
            'ship_zip'=>empty($shipping['postcode']) ? $billing['postcode'] : $shipping['postcode'],
            ...
            'cart_info'=>'Woocommerce-onepage|V1.5.0|'.$isMobile,
            'cart_api'=>'V1.0',
        );
...
//                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                        if ($this->check_order_can_update($order, $return_info['methods'])) {
                            ...
                            $order->add_order_note("oceancreditcrard return-->:"  . $testorder. $return_info['payment_details'] . '--->but not update for had paid ' . $return_info['payment_status']);
                        }
...
//                             if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
    ...
    function return_payment( $order ) {
        ...
        //返回支付
        $methods = sanitize_text_field($_REQUEST['methods'] ?? 'Credit Card');
        ...
        if(strtoupper($local_signValue) == strtoupper($back_signValue)){
            ...
            if ($payment_status == 1) {
                ...
                $order->add_order_note('ocean card number:' . $card_number);
                WC()->cart->empty_cart();
