...
if ($notices) {
    $arr_IgnoreMsg = [];
    $arr_IgnoreMsg[] = __( 'You are paying for a guest order. Please continue with payment only if you recognize this order.', 'woocommerce');
    $arr_IgnoreMsg[] = __( '50008:Transaction canceled by customer', 'woocommerce');
    $notices = array_filter($notices, function($v) use ($arr_IgnoreMsg) {
       return !in_array($v['notice'], $arr_IgnoreMsg);
    });
            ...
            <?php if ( $multiple ) { ?>
                ...
                <ul>
                    ...
                    <?php foreach ( $notices as $notice ) : ?>
                        <?php
                           if (empty($notice['notice']) || in_array($notice['notice'], $arrBeforeMsg)) {
                                continue;
                           }
                           $arrBeforeMsg[] = $notice['notice'];
                        ?>
