	...
	public function page_callback() {
        ...
        <div class="wrap">
			...
			if ( count( $results ) ) {
				...
				echo VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( $pagination_html );
				?>
                            ...
                            <tr>
								...
								foreach ( $result as $result_k => $result_v ) {
									...
									if ( $result_k === 'product_id' ) {
										?>
                                        <td><a target="_blank"
                                               href="<?php echo esc_attr( admin_url( 'post.php?action=edit&post=' . $result['product_id'] ) ) ?>"><?php esc_html_e( $product->get_title() ) ?></a>
                                        </td>
										<?php
									}
								}
