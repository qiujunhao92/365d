...
define( 'VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_VERSION', '1.2.0' );
...
if ( ! class_exists( 'IMPORT_SHOPIFY_TO_WOOCOMMERCE' ) ) {
	class IMPORT_SHOPIFY_TO_WOOCOMMERCE {
		...
		public function delete_import_history() {
			global $pagenow;
			if ( $pagenow === 'admin.php' && isset( $_POST['s2w_delete_history'] ) ) {
				...
				if ( 'import-shopify-to-woocommerce' === $page ) {
					...
					if ( isset( $_POST['products'] ) && sanitize_text_field( $_POST['products'] ) ) {
					...
					if ( isset( $_POST['product_categories'] ) && sanitize_text_field( $_POST['product_categories'] ) ) {
		...
		public function admin_notices() {
			if ( $this->process_new->is_downloading() ) {
                ...
                <div class="updated">
                    ...
                    <div>
						<?php printf( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'Please goto <a target="_blank" href="%s">Media</a> and view downloaded images. If <strong>some images are downloaded repeatedly and no new images are downloaded</strong>, please:', 'import-shopify-to-woocommerce' ) ), admin_url( 'upload.php' ), esc_url_raw( add_query_arg( array( 's2w_cancel_download_image' => '1', ) ) ) ) ?>
                        <ol>
                            <li><?php printf( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( '<strong>Stop importing products immediately</strong>', 'import-shopify-to-woocommerce' ) ) ) ?></li>
                            <li><?php printf( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( '<a class="s2w-cancel-download-images-button" href="%s">Cancel downloading</a></strong>', 'import-shopify-to-woocommerce' ) ), add_query_arg( array( 's2w_cancel_download_image' => '1', ), $_SERVER['REQUEST_URI'] ) ) ?></li>
                            <li><?php printf( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'Contact <strong>support@villatheme.com</strong> or create your ticket at <a target="_blank" href="https://villatheme.com/supports/forum/plugins/import-shopify-to-woocommerce/">https://villatheme.com/supports/forum/plugins/import-shopify-to-woocommerce/</a>', 'import-shopify-to-woocommerce' ) ) ) ?></li>
                        </ol>
                    </div>
                </div>
				<?php
                ...
                <div class="updated">
                    ...
                    <ol>
                        <li>
							<?php printf( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'If the same images are downloaded again and again, please <strong><a class="s2w-empty-queue-images-button" href="%s">Empty queue</a></strong> and go to Products to update missing images for your products.', 'import-shopify-to-woocommerce' ) ), esc_url_raw( add_query_arg( array( 's2w_cancel_download_image' => '1', ) ) ) ) ?>
                        </li>
                        <li>
							<?php printf( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'If products images were downloading normally before, please <strong><a class="s2w-start-download-images-button" href="%s">Resume download</a></strong>', 'import-shopify-to-woocommerce' ) ), esc_url_raw( add_query_arg( array( 's2w_start_download_image' => '1', ) ) ) ) ?>
                        </li>
                    </ol>
                </div>
				<?php
		...
		public function plugins_loaded() {
			...
			if ( isset( $_REQUEST['s2w_cancel_download_image'] ) && wp_unslash( sanitize_text_field( $_REQUEST['s2w_cancel_download_image'] ) ) ) {
				...
				wp_safe_redirect( esc_url_raw( remove_query_arg( 's2w_cancel_download_image' ) ) );
				exit;
			} elseif ( ! empty( $_REQUEST['s2w_start_download_image'] ) ) {
				if ( ! $this->process_new->is_queue_empty() ) {
					$this->process_new->dispatch();
				}
				wp_safe_redirect( esc_url_raw( remove_query_arg( 's2w_start_download_image' ) ) );
				exit;
			}
		}
...
//																	}
								...
								if ( $current_import_product == count( $products ) - 1 ) {
									if ( $current_import_page == $total_pages ) {
										...
										wp_send_json( array(
											'status'                 => 'finish',
											'message'                => sprintf( esc_html__( 'Completed %s/%s', 'import-shopify-to-woocommerce' ), $history['total_products'], $history['total_products'] ),
										...
										wp_send_json( array(
											'status'                 => 'successful',
											'message'                => sprintf( esc_html__( 'Importing... %s/%s completed', 'import-shopify-to-woocommerce' ), $imported_products, $history['total_products'] ),
											'imported_products'      => $imported_products,
											'current_import_id'      => $current_import_id,
											'current_import_page'    => $current_import_page,
											'current_import_product' => $current_import_product,
                                ...
                                <table class="form-table">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <label for="<?php echo esc_attr( self::set( 'domain' ) ) ?>"><?php echo VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'Your Store address, eg: <strong>myshop.myshopify.com</strong>', 'import-shopify-to-woocommerce' ) ) ?></label>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>
                                        ...
                                        <td>
                                            ...
                                            <p class="description"><?php echo VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'If you use <a target="_blank" href="https://help.shopify.com/en/manual/apps/custom-apps">custom apps</a>, please enter access token here. API key and API password are not used in this case so just ignore them.', 'import-shopify-to-woocommerce' ) ) ?></p>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="vi-ui toggle checkbox checked">
                                                    ...
                                                    <label for="<?php echo esc_attr( self::set( 'disable_background_process' ) ) ?>"><?php echo VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'Product images will be added to <a href="admin.php?page=import-shopify-to-woocommerce-error-images" target="_blank">Failed images</a> list so that you can go there to import all images with 1 click. This is recommended if your server is weak or if you usually have duplicated images issue.', 'import-shopify-to-woocommerce' ) ) ?></label>
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                ...
                <tr>
                    ...
                    <td>
						...
						if ( wp_is_writable( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_CACHE ) ) {
							echo '<mark class="yes">&#10004; <code class="private">' . VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_CACHE ) . '</code></mark> ';
						} else {
							printf( '<mark class="error">&#10005; ' . VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_DATA::wp_kses_post( __( 'To allow logging, make <code>%s</code> writable or define a custom <code>VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_CACHE</code>.', 'import-shopify-to-woocommerce' ) ) . '</mark>', VI_IMPORT_SHOPIFY_TO_WOOCOMMERCE_CACHE );
						}
						?>
